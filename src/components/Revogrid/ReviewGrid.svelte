<script>
  ...
  import { Day1, Calculate } from './Hooks'
  
  export let Data, Area, Process
...
<div class="RootContainer">
  <div></div>
  <revo-grid bind:this={Grid} class="CustomGridClass" exporting="true" autocomplete="true" style="height: {61 + (Data.length * 27)}px; align-items: center;">
    <div class="ExportButtonAligner">
      <h4 class="Title">{Area} - {Process}</h4>
      <ExportToCsvButton {Grid} FileName="{Day1} {Area} {Process} REPORT" />
    </div>
  </revo-grid>
  <div></div>
</div>
