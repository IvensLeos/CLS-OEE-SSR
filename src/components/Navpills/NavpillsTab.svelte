...
<ul class="nav nav-pills" id="myTab" role="tablist">
  {#each Machines as { MACHINE_NAME }, I}
    <li class="nav-item" role="presentation">
      <button class="nav-link {I === 0 ? "active" : ""}" id="{FormatName(MACHINE_NAME)}-{I}" data-bs-toggle="tab" data-bs-target="#{FormatName(MACHINE_NAME)}" type="button" role="tab" aria-controls={FormatName(MACHINE_NAME)} aria-selected={I === 0 ? "true" : ""}>{MACHINE_NAME}</button>
    </li>
  {/each}
</ul>
<hr>
